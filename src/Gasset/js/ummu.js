...
var $ummu = {
        ...
        dataTables: function() {
            $('#is_localstorage').on('change', function() {
                if ($(this).is(':checked')) {
                    ...
                    $("#info_localStorage_true").removeClass("collapse")
                }else{
                    localStorage.removeItem('isDataLocalStorage');
                    $("#info_localStorage_true").addClass("collapse")
                }
            });
            ...
            $("#is_tableSmall").on('change', function() {
                if ($(this).is(':checked')) {
                    localStorage.setItem('is_tableSmall', true);
                    $ummu.dt.layout.table_sm(true)
                }else{
                    localStorage.removeItem('is_tableSmall');
                    $ummu.dt.layout.table_sm(false)
                }
            });
            ...
            if (localStorage.getItem("is_tableSmall") == 'true') {
                $ummu.dt.layout.table_sm(true)
                $('#is_tableSmall').prop('checked', true)
            }else{
                $ummu.dt.layout.table_sm(false)
                $('#is_tableSmall').prop('checked', false)
            }
        },
    ...
    helpers: {
        ...
        renderComplexHeader: function(tableId, columns) {
            let row1 = '<tr>';
            let row2 = '<tr>';
            let processedGroups = [];
            ...
            columns.forEach((col) => {
                if (col.rowspan) {
                    // Kolom tunggal yang memanjang ke bawah (rowspan 2)
                    row1 += `<th rowspan="2" class="align-middle text-center">${col.title || ''}</th>`;
                } else if (col.group) {
                    // Jika kolom ini bagian dari grup (colspan)
                    if (!processedGroups.includes(col.group)) {
                        // Hitung ada berapa kolom yang memiliki nama grup yang sama
                        let groupSize = columns.filter(c => c.group === col.group).length;
                        row1 += `<th colspan="${groupSize}" class="text-center">${col.group}</th>`;
                        processedGroups.push(col.group);
                    }
                    // Masukkan judul kolom aslinya ke baris kedua
                    row2 += `<th class="text-center">${col.title}</th>`;
                }
            });
            ...
            // Suntikkan ke dalam tabel
            // $(`${tableId} thead`).html(row1 + row2);
            $(tableId).find('thead').html(row1 + row2);
        },
    ...
    controllers: {
        ...
        show_shippingInstruction: function () {
            let lcg = localStorage.getItem('shippingInstruction')

            if ($($tbListDataID).data('init') != 'shippingInstruction') {
                $tbListDataID.DataTable().destroy();
                $tbListDataID.empty(); // Opsional: bersihkan isi HTML tabel
                $ummu.dt.shippingInstruction.init = null
            }

            if (lcg) {
                if ($ummu.dt.shippingInstruction.init == null) {
                    $ummu.dt.shippingInstruction.init = new DataTable(
                        $tbListDataID, {
                        columns: [
                            {
                                title: "ID",
                            ...
                            { title: "Number SI", data: "number" },
                            { title: "Shipper", data: "client_name" },
                        ],
                        data: JSON.parse(lcg).rows,
                        layout: {
                            topStart: {
                                buttons: [
                                    ...
                                    },
                                    {
                                        ...
                                        action: function (e, dt, node, config) {
                                            ...
                                            // /*Destroy and Re-create*/
                                            $ummu.dt.shippingInstruction.init.destroy();
                                            $ummu.dt.shippingInstruction.create()
                                        },
                                    },
                                ],
                            },
                    ...
                    $ummu.dt.shippingInstruction.init.clear().rows.add(JSON.parse(lcg).rows).draw();
                }
            } else {
                if ($ummu.dt.shippingInstruction.init == null) {
                    $ummu.dt.shippingInstruction.create()
            ...
            $($tbListDataID).data('init', 'shippingInstruction');
            $ummu.dt.shippingInstruction.onClick();
        },

        show_spal: function () {
            let lcg = localStorage.getItem('spal')

            if ($($tbListDataID).data('init') != 'spal') {
                $tbListDataID.DataTable().destroy();
                $tbListDataID.empty(); // Opsional: bersihkan isi HTML tabel
                $ummu.dt.spal.init = null
            }

            if (lcg) {
                if ($ummu.dt.spal.init == null) {
                    $ummu.dt.spal.init = new DataTable(
                        $tbListDataID, {
                        columns: [
                            {
                                title: "ID",
                            ...
                            { title: "Nomor Surat Perjanjian", data: "number" },
                    ...
                    $ummu.dt.spal.init.clear().rows.add(JSON.parse(lcg).rows).draw();
                }
            } else {
                if ($ummu.dt.spal.init == null) {
                    $ummu.dt.spal.create()
            ...
            $($tbListDataID).data('init', 'spal');
            $ummu.dt.spal.onClick();
        },

        show_dept: function () {
            let lcg = localStorage.getItem('dept')

            if ($($tbListDataID).data('init') != 'dept') {
                $tbListDataID.DataTable().destroy();
                $tbListDataID.empty(); // Opsional: bersihkan isi HTML tabel
                $ummu.dt.dept.init = null
            }

            if (lcg) {
                if ($ummu.dt.dept.init == null) {
                    $ummu.dt.dept.init = new DataTable(
                        $tbListDataID, {
                        columns: [
                            {
                                title: "ID",
                    ...
                    $ummu.dt.dept.init.clear().rows.add(JSON.parse(lcg).rows).draw();
                }
            } else {
                if ($ummu.dt.dept.init == null) {
                    $ummu.dt.dept.create()
    ...
    auth: {
        load: function () {
            ...
            $($ummu.$.auth + "#btn_back").on("click", function () {
                $ummu.$.auth_alert.html("");
                // $("#form_cek_phone").removeClass("collapse");
                // $("#form_login_with_phone").addClass("collapse");
                // app.views.cek_phone();
                $($ummu.$.auth + "#form_get_otp").removeClass("collapse");
                $ummu.$.auth_phoneNumber.removeClass("collapse");
                // $("#ummu_auth #page_login_with_phone_number #btn_get_otp_email").addClass("disabled");
                // $("#ummu_auth #page_login_with_phone_number #form_cek_phone").addClass("collapse");
                if ($ummu.auth.is_password === 1) {
    ...
    ajax: {
        ...
        auth: {
            ...
            login_phoneNumber_password: function (params) {
                // console.log(params.data);
                var jqXHR = $.ajax({
                    url: params.url,
                    method: params.type,
            ...
            get_otp_email: function () {
                ...
                var url = $ummu.$.base_url + "auth/phone_number/create_otp_email";
                var params = {
                    url: url,
                    type: "post",
                    action: "create",
            ...
            get_otp_sms: function () {
                ...
                var url = $ummu.$.base_url + "auth/create_otp_sms";
                var params = {
                    url: url,
                    type: "post",
                    action: "create",
            ...
            get_otp_wa: function () {
                ...
                var url = $ummu.$.base_url + "auth/create_otp_wa";
                var params = {
                    url: url,
                    type: "post",
                    action: "create",
        ...
        // /**
        // * body = form-data 
        // * url = otomatis dari $ummu.vars.page_url tinggal tambahkan aja function berikutnya
        // * method = POST
        // * dinamis modal_loader
        // * */
        // var params = {
        ...
        ummu7b: function (params) {
            return $.ajax({
                ...
                method: "POST", //karna mimeType = "multipart/form-data", maka method wajib "POST"
                timeout: 0,
                processData: false,
                mimeType: "multipart/form-data",
                contentType: false,
        ...
        // * method dinamis dikirim dari params
        // * dinamis modal_loader
        // * contoh params di bawah
        // * */
        // /*
    ...
    func: {
        isNull: function ($element_id) {
            if ($element_id.val() == "" || $element_id.val() == null || $element_id.val() == 'undefined') {
                return true
            } else {
                return false
            }
        ...
        findMaxDataByProperty: function(array, key) {
            if (!array || array.length === 0) return null;

            return array.reduce((prev, current) => {
                // Gunakan Number() atau parseInt() untuk memastikan perbandingan angka
                const valPrev = Number(prev[key]);
                const valCurrent = Number(current[key]);

                return (valPrev > valCurrent) ? prev : current;
            });
        },

        // Fungsi pencari data
        getRowById: function(data, idBuscado) {
            // Jika data masih berupa string JSON, parse dulu. Jika sudah objek/array langsung gunakan.
            const arrayData = (typeof data === 'string') ? JSON.parse(data) : data;
            
            // Cari baris yang id-nya sama (menggunakan == agar toleran terhadap tipe data string/number)
            return arrayData.find(row => row.id == idBuscado) || null;
        },
        // // Cara Penggunaan:
        // const dataJson = '[{"id":"19",...},{"id":"20",...}]'; // data Anda
        
        // const rowWp02 = getRowById(dataJson, 20); // Mencari id 20
        // console.log(rowWp02); 
        // // Hasilnya berupa object: {"id":"20", "nama":"WP 02", "lintang":"03-23.000S", ...}
    ...
    gps: {
        ...
        calculateVoyageSummary:function(waypoints, speedKnot, consumptionPerHour) {
            ...
            for (let i = 0; i < waypoints.length - 1; i++) {
                const leg = $ummu.gps.calculateLeg(
        ...
        calculateLeg:function(p1, p2) {
            const toRad = Math.PI / 180;
            // const lat1 = $ummu.gps.parseToDD(p1.lat_raw) * toRad;
            // const lon1 = $ummu.gps.parseToDD(p1.lon_raw) * toRad;
            // const lat2 = $ummu.gps.parseToDD(p2.lat_raw) * toRad;
            // const lon2 = $ummu.gps.parseToDD(p2.lon_raw) * toRad;

            const lat1 = $ummu.gps.parseToDD(p1.lintang) * toRad;
            const lon1 = $ummu.gps.parseToDD(p1.bujur) * toRad;
            const lat2 = $ummu.gps.parseToDD(p2.lintang) * toRad;
            const lon2 = $ummu.gps.parseToDD(p2.bujur) * toRad;
        ...
        processPassagePlan:function(data) {
            ...
            for (let i = 0; i < data.waypoints.length - 1; i++) {
                ...
                const calc = $ummu.gps.calculateLeg(start, end);
        ...
        errors_msg: function (errors) {
            ...
            if (typeof errors == 'string') {
                ...
                for (const [key, value] of Object.entries(errors)) {
                    $("#modal_message #alert").append("<div><i class='fas fa-angle-right'></i> " + `${value}` + "</div>");
                }
            }
    ...
    validation: {
        ...
        inputValidate2: function() {
            ...
            inputsRequired.forEach(input => {
                ...
                if (labelElement) {
                    var teks = labelElement.innerText; // Ambil teks saja
                    var html = labelElement.innerHTML; // Ambil HTML di dalamnya
                }else{
                    var teks = '';
                    var html = '';
                }
        ...
        // Ambil text nya dari label dengan attr for = id inputnya
        // Untuk contoh format dengan urutan <label>text elementlainnya</label>
        inputValidate4: function(element_id) {
            var arrayList = [];

            // 1. Gunakan selector yang lebih luas untuk mencakup select dan textarea
            const inputsRequired = document.querySelectorAll(`#${element_id} :is(input, select, textarea)[required]`);

            inputsRequired.forEach(input => {
                const $input = $(input); // Bungkus ke jQuery sekali saja
                let labelText = "";

                // 2. Cari label berdasarkan ID input
                const $labelElement = $(`label[for="${input.id}"]`);

                if ($labelElement.length > 0) {
                    // Gunakan optional chaining atau cek childNodes untuk menghindari error
                    const firstNode = $labelElement[0].childNodes[0];
                    labelText = firstNode ? firstNode.textContent.trim() : input.id;
                } else {
                    // Fallback: Jika label tidak ketemu, gunakan name atau placeholder agar tidak kosong
                    labelText = $input.attr('placeholder') || $input.attr('name') || "Field";
                }

                // 3. Logika Validasi
                if ($input.hasClass('is-data-id')) {
                    // Cek data-id
                    if ($ummu.func.isValue($input.attr('data-id')) === false) {
                        arrayList.push(`${labelText} field is required.`);
                    }
                } else {
                    // Cek value standar
                    if ($ummu.func.isValue($input.val()) === false) {
                        arrayList.push(`${labelText} field is required.`);
                    }
                }
            });
    ...
    dt: {
        ...
        config: {
            ...
            rowGroup: function() {
                ...
                if(typeof app.dt.config.rowGroup !== "undefined") {
            ...
            initComplete: function(settings, json) {
                console.log('$ummu.dt.init installed.');
            },
            ...
            install: function(d) {
                ...
                $ummu.dt.init = new DataTable($table,{
                    language: {
                        emptyTable: "<span style=''>Silahkan klik button <span style='color: red; font-weight: bold;'>Get Data</span> untuk mengambil data terbaru</span>"
                    },
                    data: d,
                    ...
                    paging: true,
                    rowGroup: $ummu.dt.config.rowGroup(),
                    // scrollCollapse: true,
                    // scrollX: true,
                    // scrollY: '60vh',
                    initComplete: function(settings, json) {
                        if (typeof app.dt.config.initComplete !== 'undefined') {
                            app.dt.config.initComplete(settings, json)
                        }else{
                            $ummu.dt.config.initComplete(settings, json)
                        }
                    },
                    drawCallback: function (settings) {
                        // var api = this.api();
        ...
        layout: {
            ...
            table_sm: function(st) {
                if (st == true) {
                    $("#v_dataTable").addClass("table-sm text-sm");
                }else{
                    $("#v_dataTable").removeClass("table-sm text-sm");
                }
            },
            ...
            onClick: function () {
                if ($ummu.dt.clients.init !== null) {
                    $ummu.dt.clients.init.off("click").on("click", "tbody tr td:nth-child(1)", function() {
                        ...
                        $("#client").val(row.name).attr("data-id", row.id);
        ...
        shippingInstruction: {
            ...
            create: function () {
                $ummu.dt.shippingInstruction.init = new DataTable(
                    $tbListDataID,
                    $ummu.dt.shippingInstruction.config()
                );

                $ummu.dt.shippingInstruction.init.on('xhr', function () {
                    var response = $ummu.dt.shippingInstruction.init.ajax.json();
                    if (response.status == true) {
                        localStorage.setItem('shippingInstruction', JSON.stringify(response));
                    }
                });
            },
            ...
            config: function () {
                return {
                    ajax: {
                        dataSrc: "rows",
                        url: $ummu.vars.page_url + "show_si",
                        data: function (d) {
                            // d.myKey = "myValue";
                            // d.custom = $('#myInput').val();
                            // d.release = [0];
                        ...
                        topStart: {
                            buttons: [
                                ...
                                },
                                {
                                    ...
                                    className: "btn-showall-color py-1 dt-btn-ummu",
                                    action: function (e, dt, node, config) {
                                        $ummu.dt.shippingInstruction.init.ajax.reload();
                                    },
                                },
                            ],
                        },
                    ...
                    columns: [
                        ...
                        { title: "Number of SI", data: "number" },
                        { title: "Shipper", data: "client_name" },
                    ],
                    drawCallback: function (data, callback, settings) {
                        var api = this.api();
            ...
            onClick: function () {
                if ($ummu.dt.shippingInstruction.init !== null) {
                    $ummu.dt.shippingInstruction.init.off("click").on("click", "tbody tr td:nth-child(1)", function() {
                        var row = $ummu.dt.shippingInstruction.init.row(this).data();
                        // console.log(row);

                        $("#shipment").val(row.number).attr("data-id", row.id);
                        $ummu.vars.listData.selectKode = row.id;

                        $("#modal_listData").modal("hide");
                        
                        if (typeof app.controllers.on_click_tbody_trtd_child1 !== "undefined") {
                            console.log('function app.controllers.on_click_tbody_trtd_child1 is OK.');
                            app.controllers.on_click_tbody_trtd_child1(row);
                        } else {
                            console.log('plese create function app.controllers.on_click_tbody_trtd_child1.');
                        }
                    });
                }
            }
        ...
        spal: {
            ...
            create: function () {
                ...
                $ummu.dt.spal.init.on('xhr', function () {
                    var response = $ummu.dt.spal.init.ajax.json();
                    if (response.status == true) {
                        localStorage.setItem('spal', JSON.stringify(response));
                    }
                });
            },
            ...
            config: function () {
                return {
                    ajax: {
                        dataSrc: "rows",
                        url: $ummu.vars.page_url + "show_spal",
                        data: function (d) {
                            // d.myKey = "myValue";
                            // d.custom = $('#myInput').val();
                            // d.release = [0];
            ...
            onClick: function () {
                if ($ummu.dt.spal.init !== null) {
                    $ummu.dt.spal.init.off("click").on("click", "tbody tr td:nth-child(1)", function() {
                        var row = $ummu.dt.spal.init.row(this).data();
                        // console.log(row);

                        $("#spal").val(row.number).attr("data-id", row.id);
                        $ummu.vars.listData.selectKode = row.id;
                        ...
                        if (typeof app.controllers.on_click_tbody_trtd_child_spal !== "undefined") {
                            ...
                            app.controllers.on_click_tbody_trtd_child_spal(row);
                        } else {
        ...
        dept: {
            ...
            create: function () {
                ...
                $ummu.dt.dept.init.on('xhr', function () {
                    var response = $ummu.dt.dept.init.ajax.json();
                    if (response.status == true) {
                        localStorage.setItem('dept', JSON.stringify(response));
                    }
                });
            },
            ...
            config: function () {
                return {
                    ajax: {
                        dataSrc: "rows",
                        url: $ummu.vars.page_url + "show_dept",
                        data: function (d) {
                            // d.myKey = "myValue";
                            // d.custom = $('#myInput').val();
                            // d.release = [0];
            ...
            onClick: function () {
                if ($ummu.dt.dept.init !== null) {
                    $ummu.dt.dept.init.off("click").on("click", "tbody tr td:nth-child(1)", function() {
                        var row = $ummu.dt.dept.init.row(this).data();
                        // console.log(row);

                        // $("#dept").val(row.name).attr("data-id", row.id);
                        // $ummu.vars.listData.selectKode = row.id;
                        ...
                        if (typeof app.controllers.on_click_tbody_trtd_child_spal !== "undefined") {
                            console.log('function app.controllers.on_click_tbody_trtd_child_spal is OK.');
                            app.controllers.on_click_tbody_trtd_child_dept(row);
                        } else {
                            console.log('plese create function app.controllers.on_click_tbody_trtd_child_spal.');
                        }
                    });
            ...
            onClick: function () {
                if ($ummu.dt.pelabuhan.init !== null) {
                    $ummu.dt.pelabuhan.init.off("click").on("click", "tbody tr td:nth-child(1)", function() {
                        ...
                        if (typeof app.controllers.on_click_tbody_trtd_child_pelabuhan !== "undefined") {
                            console.log('function app.controllers.on_click_tbody_trtd_child_pelabuhan is OK.');
                            app.controllers.on_click_tbody_trtd_child_pelabuhan(row);
                        } else {
                            console.log('plese create function app.controllers.on_click_tbody_trtd_child_pelabuhan.');
                        }
                    });
                }
            }
    ...
    localStorage: {
        ...
        controllers: {
            ...
            updateParam_byID: function(item_name, id, keyName, newData) {
                // 1. Ambil data dari localStorage
                let rawData = localStorage.getItem(item_name);

                if (rawData) {
                    // 2. Ubah string JSON menjadi objek JavaScript
                    let data = JSON.parse(rawData);

                    // 3. Cari id "4" di dalam array 'rows' dan update detail_waypoint-nya
                    let found = false;
                    data.rows = data.rows.map(row => {
                        if (row.id == id) {
                            found = true;
                            return { ...row, [keyName]: newData };
                        }
                        return row;
                    });

                    if (found) {
                        // 4. Simpan kembali data yang sudah diupdate ke localStorage
            ...
            // versi terbaru by AI
            updateParam_byID2: function(item_name, id, keyName, newData) {
                // 1. Ambil data dari localStorage
                let rawData = localStorage.getItem(item_name);
                ...
                if (rawData) {
                    try {
                        ...
                        // Pastikan properti 'rows' ada dan merupakan array
                        if (data && Array.isArray(data.rows)) {
                            let found = false;

                            // 3. Cari rows dengan id = id di dalam array 'rows' dan update detail_waypoint-nya
                            data.rows = data.rows.map(row => {
                                // Menggunakan == agar fleksibel jika id berupa string atau number
                                if (row.id == id) {
                                    found = true;
                                    return { ...row, [keyName]: newData };
                                }
                            ...
                            if (found) {
                                // 4. Simpan kembali data yang sudah diupdate ke localStorage
                                localStorage.setItem(item_name, JSON.stringify(data));
                                console.log(`Update localStorage dengan keyName = ${keyName} pada row ID = ${id}, Berhasil!`);
                            } else {
                                console.warn(`Data dengan ID ${id} tidak ditemukan.`);
                            }
                        } else {
                            console.error("Format data localStorage tidak sesuai (rows tidak ditemukan).");
                        }
                    } catch (e) {
                        console.error("Gagal melakukan parse JSON dari localStorage:", e);
                    }
                } else {
                    console.error(`Item ${item_name} tidak ditemukan di localStorage.`);
                }
            },
        },
        ...
        sbToolbar: function () {
            ...
            $(document).on('click', '.sb-toolbar button', function () {
                ...
                if (elID == 'btn_new') {
                    ...
                    console.log('btn_cancle is save.')
    ...
    $: {
        ...
        form_login_otp: "#ummu_auth #page_login_with_phone_number #form_login_otp ",
