...
<?php 
    ...
    foreach ($rows as $key => $value) {
        ...
        // URL for parent / module
        if (isset($value['sub_module'])) {
            $modul_url = "#";
            $sub_module = $value['sub_module'];
        }else{
            $modul_url = base_url('admin/' . $value['kode']);
        }
        ...
        <?php
        if ($sub_module == null) { ?> 
            <!-- Single Parent / Single Modul -->
            <li class="nav-item">
                <a class="nav-link" href="<?= $modul_url ?>">
            ...
            <!-- Parent / Modul -->
            <li class="nav-item <?= (isset($GET_['g']) and $GET_['g'] == $value['kode']) ? 'active' : '' ?>">
                <a class="nav-link collapsed" href="<?= $modul_url ?>"
                    data-toggle="collapse" data-target="#child_<?= $value['kode'] ?>" aria-expanded="true" aria-controls="collapseUtilities" id="">
                    <?= $icon ?> <span><?= $value['name'] ?></span>
                </a>
                <div id="child_<?= $value['kode'] ?>" class="collapse" aria-labelledby="headingUtilities" data-parent="#accordionSidebar">
                    <div class="bg-white py-2 collapse-inner rounded">
                        <h6 class="collapse-header"><?=$value['category_name']?></h6>
                        <?php 
                        foreach ($sub_module as $key2 => $value2) {
                            // URL for child / sub_module
                            if (isset($value2['sub_sub_module'])) {
                                $sub_module_url = "#";
                                $sub_sub_module = $value2['sub_sub_module'];
                            }else{
                                $sub_module_url = base_url('admin/' . $value2['kode']);
                            }

                            // icon2 for child / sub_module
                            ...
                            if ($sub_sub_module == null) { ?>
                                <!-- Single Child / Single Sub Modul -->
                                <a class="collapse-item" href="<?=$sub_module_url?>" id=""><?= $value2['name'] ?></a>
                                <?php
                            } else { ?>
                                <!-- Child / Sub Modul  -->
                                <a class="nav-link2 collapsed collapse-item" href="javascript:void(0);" data-toggle="collapse" data-target="#grandchild_<?=$value2['kode']?>" aria-expanded="false" aria-controls="grandchild_<?=$value2['kode']?>">
                                ...
                                <nav class="sidenav-menu-nested nav accordion" id="accordionSidenavAppsMenu<?=$value2['kode']?>">
                                    <div class="collapse" id="grandchild_<?=$value2['kode']?>" data-parent="#accordionSidenavAppsMenu<?=$value2['kode']?>" style="">
                                        <nav class="sidenav-menu-nested nav">
                                            <?php 
                                            foreach ($sub_sub_module as $key3 => $value3) { 
                                                // URL for grandchild / sub_sub_module
                                                $sub_sub_module_url = base_url('admin/' . $value3['kode']);

                                                // icon2 for grandchild / sub_sub_module
                                                if (preg_match_all('/&[#A-Za-z0-9]+;/', $value3['icon'])) {
                                                    $icon3 = html_entity_decode($value3['icon']);
                                                } else {
                                                    $icon3 = $value3['icon'];
                                                }
                                                ?>

                                                <!-- Grandchild / Sub Sub Modul -->
                                                <a class="nav-link2 collapse-item" href="<?= $sub_sub_module_url ?>"><?=$value3['name']?></a>
                                                <?php
                                            } ?>
                                        </nav>
                                    </div>
