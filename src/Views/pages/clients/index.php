...
<?= $this->section('script') ?>
    <script>
        $ummu.vars.page_url = $base_url + 'admin/accounts/';
        var $table = $('#tbAccount')
        var $table2 = $('#tbPriv')
        var $table3 = $('#tbDismodModule')
        var $tbDismodModule = $('#tbDismodModule')
        var $remove = $('#remove')
        var selections = []
        var $crud = ["new","edit","delete"]
    </script>
    <?= $this->include('plugins/datatables/script') ?>
    <script src="<?=base_url('js/pages/account/index.js?time='.date('YmdHis'));?>"></script>
<?= $this->endSection() ?>